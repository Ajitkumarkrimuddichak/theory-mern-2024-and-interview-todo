...
const getSingleDoc = async () => {
  const result = await studentModel.findById("64107e6520a505698d85686d");
  console.log(result);
...
const getDocByField = async () => {
  const result = await studentModel.find({ name: "Ajit" });
...
const getDocByFieldSpecificField = async () => {
  const result = await studentModel.find({ name: "Ajit" }).select("name age");
